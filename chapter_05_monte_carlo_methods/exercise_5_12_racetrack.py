...
if __name__ == "__main__":
    ...
    for i in range(2):
        track_name = f'Track {track_map.capitalize()}'
        use_zero_acc = 'with zero acc.' if i else 'without zero acc.'
        key = track_name + ',' + use_zero_acc

        reward_hist, Q = off_policy_monte_carlo(total_episodes, track_map, None, i)
        reward_hist_dict[key] = reward_hist
        Q_dict[key] = Q
    
    plot_result(reward_hist_dict, total_episodes)
    with open(f'./history/exercise_5_12/track_{track_map}.pkl', 'wb') as f:
        pickle.dump(Q_dict)
