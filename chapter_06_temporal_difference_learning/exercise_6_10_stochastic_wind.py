...
def run_sarsa_windy(
    ...
    env = WindyGridworld(king_move=True)
    ...
    if plot:
        ...
        plt.savefig("./plots/exercise_6_10/step_episodes.png")
        plt.show()
        ...
        plt.savefig("./plots/exercise_6_10/rewards.png")
        plt.show()

    # render out the animation of the learned optimal path
    if render_result:
        A = Q.argmax(axis=-1)
        env = WindyGridworld(render_mode="human", king_move=True, stochastic_wind=True)
        state = env.reset()
